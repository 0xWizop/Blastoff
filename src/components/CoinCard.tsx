    ...
    >
      ...
      <div className="mb-3 flex items-start justify-between sm:mb-4">
        <div className="flex items-center gap-2.5 sm:gap-3">
          <div className="flex h-9 w-9 items-center justify-center bg-blastoff-orange/20 text-base font-bold text-blastoff-orange sm:h-10 sm:w-10 sm:text-lg">
            {token.symbol.charAt(0)}
          </div>
          <div>
            <h3 className="text-sm font-medium text-blastoff-text sm:text-base">{token.symbol}</h3>
            <p className="text-xs text-blastoff-text-secondary sm:text-sm">{token.name}</p>
          </div>
        </div>
        {socials.length > 0 && (
          <div className="flex items-center gap-1.5 sm:gap-2">
            {socials.slice(0, 2).map((s) => (
              <a
                ...
                className="flex h-8 w-8 items-center justify-center border border-blastoff-border bg-blastoff-bg text-blastoff-text-secondary transition-all active:border-blastoff-orange sm:h-9 sm:w-9 sm:hover:border-blastoff-orange sm:hover:text-blastoff-orange"
                title={s.label}
      ...
      <div className="mb-3 grid grid-cols-2 gap-2 sm:mb-4 sm:gap-3">
        ...
        <div className="bg-blastoff-bg p-2.5 sm:p-3">
          <p className="text-[10px] text-blastoff-text-muted sm:text-xs">24h Volume</p>
          <p className="font-mono text-xs text-blastoff-text sm:text-sm">
            ${(token.volume24h || 0).toLocaleString()}
          </p>
        </div>
        <div className="bg-blastoff-bg p-2.5 sm:p-3">
          <p className="text-[10px] text-blastoff-text-muted sm:text-xs">Price</p>
          <p className="font-mono text-xs text-blastoff-text sm:text-sm">
            ${token.price.toFixed(6)}
          </p>
        </div>
        <div className="bg-blastoff-bg p-2.5 sm:p-3">
          <p className="text-[10px] text-blastoff-text-muted sm:text-xs">24h Change</p>
          <p className={`font-mono text-xs sm:text-sm ${(token.priceChange24h || 0) >= 0 ? 'text-blastoff-success' : 'text-blastoff-error'}`}>
          ...
          <>
            ...
            {token.txCount24h != null && (
              <div className="bg-blastoff-bg p-2.5 sm:p-3">
                ...
                <p className="font-mono text-xs text-blastoff-text sm:text-sm">
                  {(token.txCount24h ?? 0).toLocaleString()}
                </p>
              </div>
            )}
          </>
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[10px] text-blastoff-text-muted sm:text-xs">Launched</p>
          <p className="font-mono text-xs text-blastoff-text sm:text-sm">{formatTimeAgo(token.startTime)}</p>
        </div>
        <Link
          href={`/token/${token.address}`}
          className="bg-blastoff-orange/10 px-3 py-2 text-xs font-medium text-blastoff-orange transition-all active:bg-blastoff-orange active:text-white sm:px-4 sm:text-sm sm:hover:bg-blastoff-orange sm:hover:text-white"
          onClick={(e) => e.stopPropagation()}
        >
          View Token
        </Link>
